...
def __generate_model(json_path: str, weight_range: Tuple[float]):
    ...
    for layer_id, layer_info in model_info['model_structure'].items():  # 按拓扑排序遍历
        ...
        # Keras 3 removed backend.int_shape; use the symbolic tensor shape directly.
        actual_shape = _tensor_shape(layer_dict[layer_id])
        if actual_shape != tuple(ouput_shape):
            raise Exception(f"[Debug] layer_id: {layer_id} expected shape: {tuple(ouput_shape)}  actual shape: {actual_shape}")
