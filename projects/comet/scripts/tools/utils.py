...
class ModelUtils:
    ...
    @staticmethod
    def _node_outbound_layer_name(node):
        outbound = getattr(node, "outbound_layer", None)
        if outbound is not None:
            return getattr(outbound, "name", None)
    ...
    def _node_inbound_layers(node):
        ...
        for parent in parent_nodes:
            operation = getattr(parent, "operation", None)
            if operation is not None:
    ...
    def model_copy(model, mode=''):
        ...
        if model.__class__.__name__ == 'Sequential':
            ...
            for layer in model.layers:
                ...
                if int(keras.__version__.split(".")[
                           1]) >= 7:
    ...
    def functional_model_operation(model, operation=None, suffix=None):
        """
        Apply specific operation to the model.
        Arguments:
            :model: model to be mutated.
            :operation: a dict: {layer_name, mutation_function}.
            :suffix: add suffix to the mutated model.
        Return:
            The mutated model.
        """
        from scripts.mutation.mutation_utils import LayerUtils
        import keras
        # Scan the model to extract the inside topology.
        input_layers = {}  # {A:[B]}, graph: B -> A
        output_tensors = {}  # {A: x}, graph A(tensor)=x
        model_outputs = {}  # {A: x}, graph A(tensor)=x, x is the model's output
        for layer in model.layers:
            for node in layer._outbound_nodes:
                layer_name = ModelUtils._node_outbound_layer_name(node)
        ...
        # Solve corner case when model can have multiple outputs
        model_outputs[model.layers[0].name] = model.input
        # Apply operation after specific layer.
    ...
    @staticmethod
    def functional_model_operation_symbolic_output(model, operation=None, suffix=None):
        from scripts.mutation.mutation_utils import LayerUtils
        input_layers = {}
        output_tensors = {}
        model_output = None
    ...
    @staticmethod
    def functional_model_operation_specific_layer(model, required_shape):
        pass
    ...
    def layer_divation(model, model_nodes, layer_index, layers_output_1, layers_output_2, epsilon=1e-7):
        ...
        for node in layer._inbound_nodes:
            if node not in model_nodes:
                continue
            for l in ModelUtils._node_inbound_layers(node):
        ...
        return delta_cur, (delta_cur - delta_pre) / (delta_pre + epsilon), [model.layers[i].name for i in
                                                                            input_layers_index]
    ...
    def layers_divation(model, layers_output_1, layers_output_2):
        ...
        for i in range(len(model.layers)):
            layers_divation.append(
                ModelUtils.layer_divation(model, relevant_nodes, i, layers_output_1, layers_output_2))
...
class MetricsUtils:
    ...
    @staticmethod
    def Relative_metrics(y1_pred, y2_pred):
        return [0 if np.sum(y1_pred) == 0 and np.sum(y2_pred) == 0 else np.abs(np.sum(y1_pred - y2_pred))]

    @staticmethod
    def Audee_Incon(y1_pred, y2_pred):
        return [
            0
            if (np.sum(y1_pred) == 0 and np.sum(y2_pred) == 0)
            else
            np.abs(np.sum(y1_pred[i] - y2_pred[i])) / len(y1_pred[i])
            for i in range(len(y1_pred))
        ]

    @staticmethod
    def Audee_Nan(y1_pred, y2_pred):
        return [
            0
            if (np.sum(y1_pred) == 0 and np.sum(y2_pred) == 0)
            else
            (np.max(y1_pred[i]) - np.min(y1_pred[i]) + np.max(y2_pred[i]) - np.min(y2_pred[i])) / 2
            for i in range(len(y1_pred))
        ]

    @staticmethod
    def Random(y1_pred, y2_pred, y_true):
        return [
            0
            for i in range(len(y1_pred))
        ]
    ...
    def get_all_metrics():
        metrics_dict = {}
        metrics_dict['Relative'] = MetricsUtils.Relative_metrics
        metrics_dict['D_MAD'] = MetricsUtils.D_MAD_metrics
        metrics_dict['Audee_Incon'] = MetricsUtils.Audee_Incon
        metrics_dict['Audee_Nan'] = MetricsUtils.Audee_Nan
        metrics_dict["Random"] = MetricsUtils.Random
        return metrics_dict
...
class FrameworkUtils:
    ...
    @staticmethod
    # compare two trs
    def compare(files_1, files_2):
        # files_1 is recommended to be the baseline file trs
        # files_2 is recommended to be the new file trs
        ...
        import dill
        # compare c
        new_1 = {"line": 0, "branch": 0}
        new_2 = {"line": 0, "branch": 0}
        same = {"line": 0, "branch": 0}
        for file_path in files_1.keys():
            if file_path not in files_2:
                new_1["line"] += len(files_1[file_path].lines)
                new_1["branch"] += len(files_1[file_path].branches)
                continue
            file_1 = files_1[file_path]
            file_2 = files_2[file_path]
            for line in file_1.lines.keys():
                if file_2.lines[line].hit_status is True and file_1.lines[line].hit_status is False:
                    print("compared with baseline file, new line has been found", file_path, line)
                    new_2["line"] += 1
                if file_1.lines[line].hit_status is True and file_2.lines[line].hit_status is False:
                    print("compared with baseline file, missed line has been found", file_path, line)
                    new_1["line"] += 1
                if file_2.lines[line].hit_status is True and file_1.lines[line].hit_status is True:
                    same["line"] += 1

            for branch in file_1.branches.keys():
                if file_2.branches[branch].hit_status is True and file_1.branches[branch].hit_status is False:
                    print("compared with baseline file, new branch has been found", file_path, branch)
                    new_2["branch"] += 1
                if file_1.branches[branch].hit_status is True and file_2.branches[branch].hit_status is False:
                    print("compared with baseline file, missed branch has been found", file_path, branch)
                    new_1["branch"] += 1
                if file_2.branches[branch].hit_status is True and file_1.branches[branch].hit_status is True:
                    same["branch"] += 1
        return new_1, new_2, same

    @staticmethod
    def merge_exps(exps, result):
        from classes.frameworks import Frameworks
        import sys
        import copy
        import os
        ...
        lemon = Frameworks("lemon", "", "", "")
        for c_file in result[exps[0]].c_files:
            lemon.c_files[c_file] = copy.deepcopy(result[exps[0]].c_files[c_file])
            file_1 = lemon.c_files[c_file]
            for exp in exps[1:]:
                if exp == "1" or exp == "lstm_no_cp":
                    continue
                file_2 = result[exp].c_files[c_file]
                for line in file_1.lines.keys():
                    if file_2.lines[line].hit_status is True:
                        file_1.lines[line].hit_status = True
                for branch in file_1.branches.keys():
                    if file_2.branches[branch].hit_status is True:
                        file_1.branches[branch].hit_status = True
        for py_file in result[exps[0]].py_files:
            lemon.py_files[py_file] = copy.deepcopy(result[exps[0]].py_files[py_file])
            file_1 = lemon.py_files[py_file]
            for exp in exps[1:]:
                if exp == "1" or exp == "lstm_no_cp":
                    continue
                file_2 = result[exp].py_files[py_file]
                for line in file_1.lines.keys():
                    if file_2.lines[line].hit_status is True:
                        file_1.lines[line].hit_status = True
                for branch in file_1.branches.keys():
